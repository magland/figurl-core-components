...
import BarPlotMainLayer from './BarPlotMainLayer';
const emptyDrawData = {};
const BarPlot = ({ bars, ticks, verticalLines, xLabel, onSelectRect, width, height }) => {
    const { xMin, xMax } = useMemo(() => (bars.length > 0 ? ({ xMin: bars[0].xStart, xMax: bars[bars.length - 1].xEnd }) : ({ xMin: 0, xMax: 1 })), [bars]);
    const yMax = useMemo(() => (Math.max(...bars.map(b => (b.height)))), [bars]);
    const { barBoxes, margins, pixelTicks, pixelVerticalLines } = useMemo(() => {
        const margins = {
            left: 3,
            right: 3,
            top: 5,
    ...
    return (_jsxs("div", Object.assign({ style: { width, height, position: 'relative' }, onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseUp: onMouseUp, onMouseLeave: onMouseLeave }, { children: [_jsx(BarPlotMainLayer, { barBoxes: barBoxes, margins: margins, pixelTicks: pixelTicks, pixelVerticalLines: pixelVerticalLines, xLabel: xLabel, width: width, height: height }), dragSelectCanvas] })));
};
