...
import { BaseCanvas, pointInRect } from "../figurl-canvas";
import { dragSelectReducer } from "@figurl/core-utils";
...
const draggingObjectReducer = (s, a) => {
    if (a.type === 'start') {
        // start dragging an object
        return Object.assign(Object.assign({}, s), { object: a.object, newPoint: a.point });
    }
    else if (a.type === 'end') {
        // stop dragging an object
        return Object.assign(Object.assign({}, s), { object: undefined });
    }
    else if (a.type === 'move') {
        // drag move an object
        return Object.assign(Object.assign({}, s), { newPoint: a.point });
    }
    else
        return s;
};
const Scene2d = ({ width, height, objects, onClickObject, onDragObject, onSelectObjects, onClick }) => {
    ...
    const [activeSelectRect, setActiveSelectRect] = useState();
    // The active mouse event, for purpose of passing to the event handlers (ctrlKey, shiftKey, etc)
    const [activeMouseEvent, setActiveMouseEvent] = useState();
    // handle a rect has been selected
    const handleSelectRect = useCallback((r, e) => {
    ...
    const paint = useCallback((ctxt, props) => {
        ctxt.clearRect(0, 0, width, height);
        const objectsById = {};
        for (let o of objects)
            objectsById[o.objectId] = o;
        ...
        const paintObject = (o) => {
            ...
            if ((o.type === 'line') || (o.type === 'marker')) {
                ...
                let pp = { x: o.x, y: o.y };
                if ((draggingObject) && (o.objectId === ((_a = draggingObject.object) === null || _a === void 0 ? void 0 : _a.objectId)) && (draggingObject.newPoint)) {
                    // use the new location if dragging
                    pp = draggingObject.newPoint;
                }
                if (o.type === 'line') {
                    // draw a line
                    const attributes = !o.selected ? o.attributes : o.selectedAttributes || Object.assign(Object.assign({}, o.attributes), { color: 'yellow', width: (o.attributes.width || defaultLineWidth) * 1.5 });
                    ctxt.lineWidth = attributes.width || defaultLineWidth;
                    if (attributes.dash)
                        ctxt.setLineDash(attributes.dash);
                    ...
                    // draw a marker
                    const attributes = !o.selected ? o.attributes : o.selectedAttributes || Object.assign(Object.assign({}, o.attributes), { fillColor: 'orange', radius: (o.attributes.radius || defaultMarkerRadius) * 1.5 });
                ...
                if ((obj1) && (obj2) && (obj1.type === 'marker') && (obj2.type === 'marker')) {
                    ...
                    if ((draggingObject) && (obj1.objectId === ((_b = draggingObject.object) === null || _b === void 0 ? void 0 : _b.objectId)) && (draggingObject.newPoint)) {
                        // use the new location if dragging
                        pp1 = draggingObject.newPoint;
                    }
                    let pp2 = { x: obj2.x, y: obj2.y };
                    if ((draggingObject) && (obj2.objectId === ((_c = draggingObject.object) === null || _c === void 0 ? void 0 : _c.objectId)) && (draggingObject.newPoint)) {
                        // use the new location if dragging
    ...
    return (_jsx("div", Object.assign({ style: { width, height, position: 'relative' }, onMouseDown: handleMouseDown, onMouseUp: handleMouseUp, onMouseMove: handleMouseMove, onMouseLeave: handleMouseLeave }, { children: _jsx(BaseCanvas, { width: width, height: height, draw: paint, drawData: emptyDrawData }) })));
};
const defaultDragStyle = 'rgba(196, 196, 196, 0.5)';
// check whether a point is contained in a scene2d object
const pointInObject = (o, p) => {
    if (o.type === 'marker') {
        const r = o.attributes.radius || defaultMarkerRadius;
        const R = { xmin: o.x - r, ymin: o.y - r, xmax: o.x + r, ymax: o.y + r };
